use super::astrolend_account::AstrolendAccountFixture;
use crate::{
    bank::BankFixture, astrolend_group::*, native, spl::*, transfer_hook::TEST_HOOK_ID, utils::*,
};
...
pub struct TestFixture {
    ...
    pub astrolend_group: AstrolendGroupFixture,
...
pub fn astrolend_entry(program_id: &Pubkey, accounts: &[AccountInfo], data: &[u8]) -> ProgramResult {
    astrolend::entry(program_id, unsafe { core::mem::transmute(accounts) }, data)
}
...
impl TestFixture {
    ...
    pub async fn new_with_t22_extension(
        ...
        program.add_program("astrolend", astrolend::ID, None);
        ...
        let tester_group = AstrolendGroupFixture::new(
            Rc::clone(&context),
            test_settings
                .clone()
    ...
    pub async fn create_astrolend_account(&self) -> AstrolendAccountFixture {
        AstrolendAccountFixture::new(Rc::clone(&self.context), &self.astrolend_group.key).await
    }
